...
// --- SOVEREIGN BLOCK v2.4 (SINCERE CORE) ---
struct SovereignBlock {
    ...
    void train(float* in, float dL_dout, float* dL_din, float lr) {
        ...
        // --- GRADIENT ACCUMULATION (SINCERE) ---
        float dL_da1[8], dL_dw_feat[8], dL_db_feat;
        dL_db_feat = dL_dy_raw;
        for (int i = 0; i < 8; i++) {
        ...
        float dL_dw1[8][3], dL_db1[8];
        for (int i = 0; i < 8; i++) {
            float dL_dz1 = dL_da1[i] * relu_derivative(z1[i]);
            dL_db1[i] = dL_dz1;
            float ff_in[3] = {in[0], in[1], y_att};
            for (int j = 0; j < 3; j++) {
                dL_dw1[i][j] = dL_dz1 * ff_in[j];
                // Using w1 (still at original state) for backprop
        ...
        float dL_dwA[HEADS][3], dL_dwB[HEADS][3], dL_dwS[HEADS][3];
        ...
        for (int k = 0; k < HEADS; k++) {
            ...
            for (int j = 0; j < 3; j++) {
                dL_dwA[k][j] = dL_df1 * in[j];
                dL_dwB[k][j] = dL_df2 * in[j];
                dL_dwS[k][j] = dL_dS_k * in[j];
                // Using original project weights (wA, wB, wS)
                dL_din_total[j] += dL_df1 * wA[k][j] + dL_df2 * wB[k][j] + dL_dS_k * wS[k][j];
            }
        }

        // --- PARAMETER UPDATES (DEFERRED) ---
        b_feat += lr * dL_db_feat;
        for (int i = 0; i < 8; i++) {
            w_feat[i] += lr * dL_dw_feat[i];
            b1[i] += lr * dL_db1[i];
            for (int j = 0; j < 3; j++) w1[i][j] += lr * dL_dw1[i][j];
        }
        for (int k = 0; k < HEADS; k++) {
            for (int j = 0; j < 3; j++) {
                wA[k][j] += lr * dL_dwA[k][j];
                wB[k][j] += lr * dL_dwB[k][j];
                wS[k][j] += lr * dL_dwS[k][j];
            }
        }
...
int main() {
    ...
    std::cout << "--- SOVEREIGN v2.4 (SINCERE G-FLOW) ---\n\n";
    ...
    for (int epoch = 0; epoch < 10001; epoch++) {
        ...
        for (auto &p : data) {
            ...
            float dL_dh2[3];
            float dL_dw_final[3], dL_db_final;
            dL_db_final = error;
            for(int i=0; i<3; i++) {
                dL_dw_final[i] = error * h2[i];
                dL_dh2[i] = error * w_final[i];
            }
            
            // Final Head Update
            for(int i=0; i<3; i++) w_final[i] += lr * dL_dw_final[i];
            b_final += lr * dL_db_final;
